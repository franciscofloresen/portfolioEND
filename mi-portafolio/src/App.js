...
const Chatbot = () => {
    ...
    // Tu API endpoint de Terraform
    const API_ENDPOINT = "https://6ivpllgw70.execute-api.us-east-1.amazonaws.com/chat";
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            const botResponse = { sender: 'bot', text: data.response || "Sorry, I couldn't get a response." };
            setMessages(prev => [...prev, botResponse]);

        } catch (error) {
            console.error("Error fetching bot response:", error);
            const errorResponse = { sender: 'bot', text: "Sorry, I'm having trouble connecting to my brain right now. Please try again later." };
